...
extract_best_parameters_par <- function(output, responseName_brts, distrib){
  ...
  # add bag fraction
  df %>%
    dplyr::mutate(bag.fract = as.numeric(substr(df$model, nchar(as.character(df$model)), nchar(as.character(df$model))))) -> df
...
partial_dependance_plots3 <- function(model, responseName_brts, distrib){
  ...
  for (j in 1:length(predictors)){
    ...
    plot(a, ylab="Coral cover", pch = 16, cex.axis=1.2, cex.lab=1.5, cex = 1)
